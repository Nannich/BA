...
```bash
python3 main.py trajectory train dyn-LL-2000-5 --gene all --loss mse --model kan
```
...
### Extract GRN Edges
Infers GRNs, architecture can be cusomized.
...
```bash
python3 main.py symbolic extract dyn-LL-2000-5 --mode grn --arch smo_log_l
```
* `--mode`: Which task KAN to extract from. Options: `grn` (default), `trajectory`.
* **File Storage Mapping:**
...
Evaluates GRN inference performance across architectures.

```bash
python3 main.py benchmark data/raw/BEELINE-data/inputs/Synthetic
```
* **File Storage Mapping:**
  ...
  * **Saves:** Comparison CSVs at `results/benchmark/<search_path_folder_name>/` containing `auroc_individual.csv`, `auroc_median.csv`, `auprc_individual.csv`, `auprc_median.csv`, `auprc_ratio_individual.csv`, and `auprc_ratio_median.csv`.
